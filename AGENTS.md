...
By default `run.sh`/`exec.sh` run the container as the invoking host user (same UID/GID and full group list via `--group-add`, computed from `id -G`), mounting `/etc/passwd` and `/etc/group` read-only and the host `/home`, so identities and groups resolve. `$HOME` is an isolated per-user workspace directory (default `~/.jnl-dl-container-home`, override with `--workspace-home`), not the real host home, so container-side changes never touch real host dotfiles. All default dotfiles (`.bashrc`, `.bash_profile`, `.inputrc`, `.tmux.conf`, `.vimrc`, `.env_setup.sh`) live in one directory, `/etc/jnl-dl/skel`, and are copied into `$HOME` automatically and idempotently (copy-if-missing, like `/etc/skel` for `useradd -m`) by `/etc/jnl-dl/bootstrap.sh` — hooked from `/etc/bash.bashrc`/`/etc/profile.d`, not `entrypoint.sh`, so it works under any runtime that runs a plain bash shell (Docker exec, Apptainer shell/exec, `docker run --entrypoint bash`, ...), and identically for root (no build-time special case). `~/.ssh`/`~/.gitconfig` are symlinked in the same automatic, self-healing way. `$ENV_SETUP_FILE` always resolves to `~/.env_setup.sh`, guaranteed to exist from the first shell, no staleness. `ADOPT_DEFAULT_CONFIGS` (defined in `$ENV_SETUP_FILE`) force re-syncs the shipped skel defaults over your own edits, backing them up first — for deliberate updates only, not first-time setup. Passwordless `sudo` is available. Other per-user helpers: `INSTALL_VCPKG`, `INSTALL_AI_CLI` (codex + claude into a personal npm prefix).
