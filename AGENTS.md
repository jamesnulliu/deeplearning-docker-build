...
By default `run.sh`/`exec.sh` run the container as the invoking host user (same UID/GID and full group list via `--group-add`, computed from `id -G`), mounting `/etc/passwd` and `/etc/group` read-only and the host `/home`, so identities and groups resolve. `$HOME` is an isolated per-user workspace directory (default `~/.dl-workspace`, override with `--workspace-home`), not the real host home — `entrypoint.sh` seeds it once (copy-if-missing) with the default dotfiles and symlinks in `~/.ssh`/`~/.gitconfig` from the real host home, so container-side changes never touch real host dotfiles. The toolchain env (`$ENV_SETUP_FILE`) is sourced from that seeded `~/.bashrc`, not a system-wide hook. Passwordless `sudo` is available. Per-user helpers defined in `$ENV_SETUP_FILE`: `INSTALL_VCPKG`, `INSTALL_AI_CLI` (codex + claude into a personal npm prefix), and `ADOPT_DEFAULT_CONFIGS` (re-sync the `CONTAINER_DEFAULT_*` dotfiles into `$HOME` after the first automatic seed).
